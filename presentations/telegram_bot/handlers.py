...
async def compare_price_specific_crypto_ticker(message: Message, state: FSMContext):
    if bot_user_service.check_active_user(tg_id = str(message.from_user.id)):
        state.set_state(ComparingPrice.ticker)
        message.answer("Напишите тикер(сокращенное название) нужной вам криптовалюты(например, BTC)")
    else:
        await state.clear()
        await message.answer("Зарегистрируйтесь для использования этой функции")


@router.message(ComparingPrice.ticker)
async def compare_price_specific_crypto(message: Message, state: FSMContext):
    crypto = await crypto_service.compare_price_specific_crypto(ticker = str(message.text).upper())
    await message.answer(f"Самая выгодная цена = {round(float(crypto[0][1]), 5)}, самая низкая цена = {round(float(crypto[-1][1]), 5)}, потенциальная выгода = {round(float(crypto[0][1]) - float(crypto[-1][1]), 5)}")
    await state.clear()
...
async def get_price_specific_crypto(message: Message, state: FSMContext):
    try:
        ...
        answer = (str(round(float(price["binance"]), 5)) + "$ (USD)")
    except Exception:
        answer = "Тикер не распознан"
    finally:
...
async def reg_password(message: Message, state: FSMContext):
    async with asyncio.TaskGroup() as tg:
        ...
        if str(await tg.create_task(send_email(getter = message.text, salt = gen_salt))) == "Message was not sent":
            ...
            await message.answer("На почту отправлена ссылка, перейди по ней и отправь боту сгенерированное сообщение")
